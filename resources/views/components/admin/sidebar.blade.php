<div class="sidebar flex flex-col h-full text-slate-200 transition-all duration-300 ease-in-out">

    {{-- Company Switcher / Brand --}}
    <div class="flex items-center gap-3 px-5 py-5 border-b border-slate-800 transition-all duration-300">
        <div class="w-9 h-9 bg-gradient-to-tr from-indigo-500 to-violet-600 rounded-xl flex items-center justify-center shadow-md ring-1 ring-black/20 shrink-0">
        ...
        <div x-show="!collapsed" class="transition-opacity duration-300 overflow-hidden whitespace-nowrap">
            <p class="text-slate-100 font-semibold text-sm tracking-tight leading-none">APG CMS</p>
            <p class="text-slate-400 text-xs mt-0.5">Multi-Company</p>
        </div>
    </div>

    {{-- Company switcher quick select --}}
    <div class="px-4 py-3 border-b border-slate-800 bg-slate-900/60">
        <div class="flex items-center gap-2">
            <select class="w-full text-sm bg-slate-800 border border-slate-700 text-slate-200 rounded-lg px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400/20">
    ...
    {{-- Navigation --}}
    <nav class="flex-1 py-5 space-y-1 overflow-y-auto overflow-x-hidden px-3">
        
        <div class="px-2 text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2 mt-2">General</div>
    ...
    {{-- User / Footer Section --}}
    <div class="mt-auto border-t border-slate-800 bg-slate-900/60 transition-all duration-300 p-4">
        <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                <div class="relative flex-shrink-0">
                    <div class="w-9 h-9 rounded-full bg-indigo-600/10 flex items-center justify-center text-indigo-300 text-sm font-semibold ring-1 ring-black/20">
                        {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 1)) }}
                    </div>
                </div>
                <div class="hidden lg:flex flex-col">
                    <p class="text-slate-100 text-sm font-medium truncate">{{ auth()->user()->name ?? 'Guest' }}</p>
            ...
            <div class="flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="p-2 text-slate-200 hover:text-rose-400 rounded-md hover:bg-rose-900/20 transition-colors" title="Sign Out">
