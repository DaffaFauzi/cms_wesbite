...
<a href="{{ $href }}" class="group flex items-center gap-3 px-3 py-2.5 rounded-xl border border-transparent hover:border-slate-200/60 hover:bg-slate-50 hover:shadow-sm transition-all duration-200">
    <div class="w-8 h-8 rounded-lg flex items-center justify-center ring-1 transition-colors {{ $colorClass }}">
        <div class="w-4 h-4">
            {{ $icon ?? '' }}
        </div>
    </div>
    <span class="text-sm font-medium text-slate-700 group-hover:text-slate-900">{{ $title }}</span>
    <div class="ml-auto opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all text-slate-400 group-hover:text-slate-600">
