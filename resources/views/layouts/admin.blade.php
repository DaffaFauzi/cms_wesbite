...
<div class="min-h-screen bg-slate-50" x-data="{ mobileOpen: false, collapsed: false }">
    <div class="flex">

        {{-- Sidebar --}}
        <aside :class="mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'" class="fixed inset-y-0 left-0 z-40 w-72 bg-slate-950 border-r border-slate-800 transition-transform duration-200 ease-in-out">
            <x-admin.sidebar />
        </aside>

        {{-- Main wrapper --}}
        <div class="min-h-screen flex-1 ml-72 min-w-0">
            {{-- Mobile overlay --}}
        ...
        {{-- PAGE CONTENT --}}
        <main class="flex-1 w-full pt-4 pb-10">
            <div class="max-w-[1600px] mx-auto w-full px-8 sm:px-6 lg:px-8">
                <div class="space-y-6">
